...
CREATE UNIQUE INDEX uq_team_stat_values_unique_active
    ON team_stat_values (
        league_public_id,
        season_ref_id,
        team_public_id,
...
CREATE INDEX idx_team_stat_values_lookup_team_active
    ON team_stat_values (league_public_id, season_ref_id, team_public_id, external_team_id, stat_key, scope)
    WHERE deleted_at IS NULL;

CREATE INDEX idx_team_stat_values_lookup_fixture_active
    ON team_stat_values (league_public_id, fixture_public_id, external_fixture_id, stat_key, scope)
    WHERE deleted_at IS NULL;

CREATE TRIGGER trg_team_stat_values_touch_updated_at
    BEFORE UPDATE ON team_stat_values
    FOR EACH ROW
