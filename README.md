...
- `UPTRACE_DSN` (required when `UPTRACE_ENABLED=true`, unless provided via `OTEL_EXPORTER_OTLP_HEADERS`)
- `UPTRACE_LOGS_ENABLED` (default `true`; mirrors structured zap logs to OpenTelemetry logs)
- `OTEL_EXPORTER_OTLP_HEADERS` (optional; supports `uptrace-dsn=...` fallback when `UPTRACE_DSN` is empty)
- `BETTERSTACK_ENABLED` (default `false`)
- `BETTERSTACK_ENDPOINT` (required when `BETTERSTACK_ENABLED=true`; accepts full URL or host only, e.g. `s1765114.eu-fsn-3.betterstackdata.com`)
- `BETTERSTACK_TOKEN` (optional bearer token for Better Stack source)
- `BETTERSTACK_TIMEOUT` (default `3s`)
- `BETTERSTACK_MIN_LEVEL` (default `error`)
